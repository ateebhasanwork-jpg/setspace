...
async function getVersionWithUploader(versionId: number) {
  ...
  const [uploader] = await db.select().from(usersTable).where(eq(usersTable.id, version.uploadedById));
  return {
    ...version,
...
router.get("/video-projects", async (req, res) => {
  try {
    ...
    if (req.query.taskId) {
      return res.json(result.filter(p => p.taskId === parseInt(req.query.taskId as string)));
    }
...
router.post("/video-versions/:versionId/share-token", async (req, res) => {
  try {
    ...
    // Upsert: delete old token for this version and create new one
    ...
    // Update version with share token
...
router.get("/review/:token", async (req, res) => {
  try {
    ...
    if (!shareToken) return res.status(404).json({ error: "Invalid or expired link" });

    const [version] = await db.select().from(videoVersionsTable).where(eq(videoVersionsTable.id, shareToken.versionId));
    if (!version) return res.status(404).json({ error: "Video not found" });
